    ...
    <div className="min-h-screen bg-[#060814] text-white relative overflow-hidden">
      ...
      <header className="relative z-10 border-b border-white/5">
        <div className="container flex h-16 items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-bold text-white">أرباحي</h1>
          </div>
        </div>
      </header>
      ...
      <main className="relative z-10 container py-8 max-w-4xl">
        ...
        <div className="rounded-3xl bg-white/[0.03] border border-teal-500/20 backdrop-blur-sm p-8 mb-6 text-center">
          <p className="text-sm text-white/40 mb-2">الرصيد الحالي (غير مسوّى)</p>
          <p className="text-4xl font-extrabold text-teal-300 mb-6">
            {(balance.data?.amount ?? 0).toLocaleString()} د.ع
          </p>
          <div className="flex items-center justify-center gap-6">
            <div className="flex items-center gap-2 text-sm">
              <CheckCircle2 className="w-4 h-4 text-emerald-400" />
              <span className="text-white/60">واصلة غير مسوّاة: <span className="text-white font-semibold">{balance.data?.deliveredCount ?? 0}</span></span>
            </div>
            <div className="flex items-center gap-2 text-sm">
              <XCircle className="w-4 h-4 text-red-400" />
              <span className="text-white/60">ملغية/مرتجعة غير مسوّاة: <span className="text-white font-semibold">{balance.data?.cancelledCount ?? 0}</span></span>
            </div>
          </div>
        </div>
        ...
        <Tabs defaultValue="current" className="w-full">
          <TabsList className="grid w-full grid-cols-2 mb-6 bg-white/5 border border-white/10 rounded-xl p-1">
            ...
            <TabsTrigger value="history" className="rounded-lg data-[state=active]:bg-emerald-600 data-[state=active]:text-white text-white/50">
              ...
              سجل التسويات ({history.data?.length ?? 0})
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="current">
            {balance.isLoading ? (
                    ...
                    <div className="flex items-center justify-between gap-4">
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <Badge className="bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">تم التسليم</Badge>
                        </div>
                        <p className="text-sm text-white/60 mt-1">{row.productType}</p>
                      </div>
                      <div className="text-start shrink-0">
                        <p className="text-lg font-bold text-teal-300">{row.amount.toLocaleString()} د.ع</p>
              ...
              <div className="text-center py-12">
                ...
                <p className="text-white/40">لا يوجد رصيد غير مسوّى حالياً</p>
              </div>
            )}
          </TabsContent>
          ...
          <TabsContent value="history">
            {history.isLoading ? (
                  ...
                  <div key={settlement.id} className="rounded-2xl bg-white/[0.03] border border-white/10 p-5 backdrop-blur-sm">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 space-y-1">
                        <div className="flex items-center gap-2">
                          <Badge className="bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">تم تسليم الأرباح</Badge>
                          <span className="text-xs text-white/30">{new Date(settlement.createdAt).toLocaleDateString("ar-IQ")}</span>
                        </div>
                        <p className="text-sm text-white/60">
                          <span className="text-white/40">واصلة:</span> {settlement.deliveredCount}
                          <span className="text-white/40 ms-3">ملغية/مرتجعة:</span> {settlement.cancelledCount}
                        </p>
                        {settlement.note && (
                          <p className="text-sm text-white/60"><span className="text-white/40">ملاحظة:</span> {settlement.note}</p>
                        )}
                      </div>
                      <div className="text-start shrink-0">
                        <p className="text-lg font-bold text-teal-300">{settlement.amount.toLocaleString()} د.ع</p>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="text-center py-12">
                ...
                <p className="text-white/40">لا يوجد سجل تسويات بعد</p>
              </div>
            )}
          </TabsContent>
        </Tabs>
